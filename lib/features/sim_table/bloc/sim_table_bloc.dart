...
import 'sim_table_event.dart';
import 'sim_table_state.dart';

const _uuid = Uuid();

class SimTableBloc extends HydratedBloc<SimTableEvent, SimTableState> {
  SimTableBloc() : super(SimTableState.initial()) {
  ...
  @override
  SimTableState? fromJson(Map<String, dynamic> json) {
    try {
      return SimTableState.fromJson(json);
    } catch (_) {
      return SimTableState.initial();
    }
  }

  @override
  Map<String, dynamic>? toJson(SimTableState state) {
    try {
      return state.toJson();
    } catch (_) {
      return null;
  ...
  void _onInitializeGame(InitializeGame event, Emitter<SimTableState> emit) {
  ...
  void _onStartRound(StartRound event, Emitter<SimTableState> emit) {
  ...
  void _onSelectCurrentPlayer(
      SelectCurrentPlayer event, Emitter<SimTableState> emit) {
    emit(state.copyWith(
  ...
  void _onPlayerPass(PlayerPass event, Emitter<SimTableState> emit) {
    final session = state.session;
  ...
  void _onSelectPassCauser(
      SelectPassCauser event, Emitter<SimTableState> emit) {
    final session = state.session;
    final causer =
  ...
  void _onDismissPassCauserSelection(
      DismissPassCauserSelection event, Emitter<SimTableState> emit) {
    emit(state.copyWith(
      session: state.session.copyWith(
  ...
  void _onSelectStoneForDistribution(
      SelectStoneForDistribution event, Emitter<SimTableState> emit) {
    final session = state.session;
  ...
  void _onDistributeSmallStone(
      DistributeSmallStone event, Emitter<SimTableState> emit) {
  ...
  void _onDistributeBigStone(
      DistributeBigStone event, Emitter<SimTableState> emit) {
    _executeDistribution(
  ...
  void _onDropStoneToPlayer(
      DropStoneToPlayer event, Emitter<SimTableState> emit) {
    _executeDistribution(
  ...
  void _onOpenRoundSettlement(
      OpenRoundSettlement event, Emitter<SimTableState> emit) {
    emit(state.copyWith(
      session: state.session.copyWith(
  ...
  void _onSelectRoundWinner(
      SelectRoundWinner event, Emitter<SimTableState> emit) {
    final session = state.session;
  ...
  void _onConfirmSettlement(
      ConfirmSettlement event, Emitter<SimTableState> emit) {
    final session = state.session;
  ...
  void _onResolveHansipTie(
      ResolveHansipTie event, Emitter<SimTableState> emit) {
    final session = state.session;
    final preview = session.settlementPreview;
    if (preview == null) return;
  ...
  void _applySettlement(Emitter<SimTableState> emit, settlementPreview) {
    final session = state.session;
  ...
  void _onStartNextRound(StartNextRound event, Emitter<SimTableState> emit) {
    final session = state.session;
    final stock = GameRuleUtils.createInitialStock(session.totalStonesConfig);
    final resetPlayers = session.players
        .map((p) => p.copyWith(
  ...
  void _onUndoLastAction(UndoLastAction event, Emitter<SimTableState> emit) {
  ...
  void _onResetRound(ResetRound event, Emitter<SimTableState> emit) {
    final session = state.session;
    final stock = GameRuleUtils.createInitialStock(session.totalStonesConfig);
    final resetPlayers = session.players
        .map((p) => p.copyWith(
  ...
  void _onResetGame(ResetGame event, Emitter<SimTableState> emit) {
    emit(SimTableState.initial());
  }

  // ── ClearMessage ─────────────────────────────────────────────────────────────

  void _onClearMessage(ClearMessage event, Emitter<SimTableState> emit) {
    emit(state.copyWith(
      session: state.session.copyWith(
        clearError: true,
        clearSuccess: true,
  ...
  void _onCancelRoundSettlement(
      CancelRoundSettlement event, Emitter<SimTableState> emit) {
    final session = state.session;
